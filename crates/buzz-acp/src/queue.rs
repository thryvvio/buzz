...
impl EventQueue {
    ...
    /// Monotonically extend an existing in-flight deadline for `scope`.
    ///
    ...
    /// moves backward. If the scope is not in-flight (already completed
    ...
    pub fn set_retry_count_for_test(&mut self, channel_id: Uuid, count: u32) {
        self.retry_counts
            .insert(ConversationSessionKey::channel(channel_id), count);
    }
...
mod tests {
    ...
    fn extend_in_flight_deadline_advances_existing_entry() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let scope = ConversationSessionKey::channel(ch);
        let old_deadline = Instant::now() + Duration::from_secs(100);
        q.in_flight_scopes.insert(scope.clone());
        q.in_flight_deadlines.insert(scope.clone(), old_deadline);

        q.extend_in_flight_deadline(&scope, 7200);
        let new = *q.in_flight_deadlines.get(&scope).unwrap();
        assert!(
            new > old_deadline,
    ...
    fn extend_in_flight_deadline_is_monotonic() {
        ...
        q.in_flight_scopes.insert(scope.clone());
        q.in_flight_deadlines.insert(scope.clone(), far_future);

        q.extend_in_flight_deadline(&scope, 7200);
        let after = *q.in_flight_deadlines.get(&scope).unwrap();
    ...
    fn extend_in_flight_deadline_noop_after_mark_complete() {
        ...
        q.in_flight_scopes.insert(scope.clone());
        q.in_flight_deadlines
            .insert(scope.clone(), Instant::now() + Duration::from_secs(100));
        q.in_flight_batch_sizes.insert(scope.clone(), 1);

        q.mark_complete(&scope);
        assert!(!q.in_flight_deadlines.contains_key(&scope));

        q.extend_in_flight_deadline(&scope, 7200);
        assert!(
            !q.in_flight_deadlines.contains_key(&scope),
    ...
    fn compact_expired_state_preserves_extended_in_flight_deadline() {
        ...
        q.in_flight_scopes.insert(scope.clone());
        q.in_flight_deadlines.insert(scope.clone(), extended);
        ...
        assert_eq!(
            *q.in_flight_deadlines.get(&scope).unwrap(),
            extended,
    ...
    fn extended_deadline_prevents_flush_next_expiry() {
        ...
        q.in_flight_scopes.insert(scope.clone());
        q.in_flight_deadlines
            .insert(scope.clone(), Instant::now() + Duration::from_secs(9999));
        q.in_flight_batch_sizes.insert(scope.clone(), 1);
        ...
        assert!(
            q.in_flight_scopes.contains(&scope),
    ...
    fn repeated_extend_in_flight_deadline_is_monotonic() {
        ...
        q.extend_in_flight_deadline(&scope, 7200);
        let after_first = *q.in_flight_deadlines.get(&scope).unwrap();

        q.extend_in_flight_deadline(&scope, 7200);
        let after_second = *q.in_flight_deadlines.get(&scope).unwrap();
